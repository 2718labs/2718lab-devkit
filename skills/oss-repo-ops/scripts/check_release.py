...
本脚本自动识别 AstrBot 插件仓库(`metadata.yaml`)或 Codex 插件仓库
(`.codex-plugin/plugin.json`)并核对发布机械项。AstrBot 代码校验仍由
astrbot-plugin-dev 的 validate_plugin.py 负责。
...
def load_metadata(repo: Path):
    ...
    try:
        ...
        warn(
            "META",
            "未安装 PyYAML,退回正则解析(pip install pyyaml 可获得完整校验,尤其是 version 浮点数陷阱)",
        )
    return data, text


def load_codex_manifest(repo: Path) -> dict | None:
    ...
    try:
        ...
        err("CODEX_META", f".codex-plugin/plugin.json 无法解析: {exc}")
        return {}
    if not isinstance(data, dict):
        err("CODEX_META", ".codex-plugin/plugin.json 顶层必须是 JSON object")
        return {}
    return data


def check_codex_manifest(data: dict, repo: Path) -> str | None:
    required = ("name", "version", "description", "author", "repository", "license")
    missing = [key for key in required if not data.get(key)]
    if missing:
        err("CODEX_META", f".codex-plugin/plugin.json 缺少必填字段: {missing}")

    name = data.get("name")
    if isinstance(name, str) and name != repo.resolve().name:
        warn(
            "CODEX_NAME",
            f"manifest name={name!r} 与目录名 {repo.resolve().name!r} 不一致",
        )

    version = data.get("version")
    if not isinstance(version, str) or not CODEX_SEMVER_RE.fullmatch(version):
        err(
            "CODEX_VER",
            f"plugin version={version!r} 不是严格 MAJOR.MINOR.PATCH SemVer",
        )
        return None

    author = data.get("author")
    if not isinstance(author, dict) or not author.get("name"):
        err("CODEX_META", "plugin author.name 缺失")

    repository = data.get("repository")
    if isinstance(repository, str) and repository.endswith(".git"):
        warn("CODEX_REPO", "plugin repository 建议不带 .git 后缀")
    return version
...
def check_version(data, text: str) -> str | None:
    ...
    if data is not None and isinstance(data, dict) and "version" in data:
        ...
        if not isinstance(v, str):
            err(
                "VER",
                f"metadata.yaml 的 version 被解析成 {type(v).__name__}({v!r}) —— 非字符串会让市场校验挂。"
    ...
    if not version.startswith("v"):
        warn(
            "VER",
            f"version={version!r} 未带 v 前缀 —— 市场接受纯 3 段号,团队建议 vX.Y.Z 以便与 git tag / Release 三处一致",
        )
    if not re.match(r"^\d+\.\d+\.\d+([-+.].+)?$", bare):
        warn(
            "VER",
            f"version={version!r} 不是标准 MAJOR.MINOR.PATCH(2 段号会被 YAML 当浮点数),确认是否有意为之",
        )
    return version
...
def check_astrbot_version(data, text: str) -> None:
    if data is not None and isinstance(data, dict) and "astrbot_version" in data:
        ...
        if not isinstance(av, str):
            err(
                "AVER",
                f"astrbot_version 被解析成 {type(av).__name__}({av!r}) —— 必须加引号成字符串",
            )
            return
        value = av
    else:
        ...
        if raw is None:
            warn(
                "AVER",
                "metadata.yaml 未声明 astrbot_version(建议按用到的 API 明确下界)",
            )
            return
        if not (raw.startswith('"') or raw.startswith("'")):
            err(
                "AVER",
                f"astrbot_version 值 {raw!r} 未加引号 —— 区间字符串必须带引号,否则 YAML 解析异常",
            )
        value = raw.strip("\"'")
    ...
    if value.lstrip().startswith("v"):
        err(
            "AVER",
            f'astrbot_version={value!r} 不应带 v 前缀(与 version 字段正好相反,PEP 440 区间如 ">=4.16,<5")',
        )
...
def check_repo_field(data, text: str) -> None:
    ...
    if str(r).endswith(".git"):
        warn(
            "REPO",
            f"repo={r!r} 以 .git 结尾 —— 官方示例一律不带,建议去掉(市场 schema 据称也接受 .git,但不带更稳妥)",
        )
...
def check_name(data, text: str, repo: Path) -> None:
    ...
    if not re.match(r"^[a-z0-9_]+$", ns):
        err(
            "NAME",
            f"name={n!r} 只能是小写字母/数字/下划线(不能有连字符/大写/空格)—— 它会成为插件加载的模块名",
        )
    elif not ns.startswith("astrbot_plugin_"):
        warn(
            "NAME",
            f"name={n!r} 缺 astrbot_plugin_ 前缀 —— 官方推荐使用,团队约定建议遵守",
        )
    dirname = repo.resolve().name
    if str(n) != dirname:
        warn(
            "NAME",
            f"name={n!r} 与目录名 {dirname!r} 不一致(市场要求相等;若目录只是本地检出名可忽略)",
        )
...
def check_changelog_has_version(repo: Path, version: str | None) -> None:
    ...
    if version not in body and bare not in body:
        warn(
            "CHLOG",
            f"CHANGELOG.md 未见当前版本条目({version} 或 {bare}) —— 用户在 WebUI 更新只能靠它",
        )
...
def check_ci(repo: Path) -> None:
    ...
    if not wf.is_dir() or not any(wf.glob("*.y*ml")):
        warn(
            "CI", ".github/workflows/ 下没有 CI 文件(建议照抄 assets/templates/ci.yml)"
        )
...
def check_size(repo: Path) -> None:
    ...
    if total > MAX_ZIP_BYTES:
        err(
            "SIZE",
            f"剔除 .git/__pycache__ 等后约 {mb:.1f}MB,超市场 16MB 上限,CI 会自动拒。先瘦身(见 references/astrbot-market.md 第3节)",
        )
...
def main() -> int:
    ...
    codex_manifest = load_codex_manifest(repo)
    version = None
    ...
    elif codex_manifest is not None:
        version = check_codex_manifest(codex_manifest, repo)
    else:
        err(
            "META",
            "未找到 metadata.yaml 或 .codex-plugin/plugin.json,无法判定发布类型",
        )
