...
host 只消费 `action="start"` descriptor，绝不重新 spawn `action="retain"`；仅在终态事件后 refill，且没有安全有用的工作时必须如实保留 idle slot。不得按 commentary 更新轮询或补位（no commentary polling）。路由下限为：
