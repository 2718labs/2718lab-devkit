...
1. The coordinator reads the task card, direct contracts, and declared write scope.
...
6. The worker creates a candidate commit and hands its evidence to the coordinator.
7. The coordinator requests independent review only when the exact route
   requires it, orders integration, runs the CI gate, and accepts or releases
   the result.
