...
class _IndexStub:
    def __init__(self, workspace: Path, snapshot_id: str) -> None:
        ...
        self.assertions: list[tuple[str, str, tuple[str, ...], bool]] = []
    ...
    def assert_current(
        ...
        self.assertions.append(
            (canonical.as_posix(), snapshot_id, scope, allow_absent_paths)
        )
...
class StrictProjectIndexWorkflowTests(unittest.TestCase):
    ...
    def test_strict_write_registration_preserves_exact_planned_file_scope(
        self,
    ) -> None:
        (self.workspace / ".git").write_text(
            "gitdir: D:/linked/worktree\n", encoding="utf-8"
        ...
        registered = self._register(
            "planned-write",
            write_scope=("src/new_module.py", "tests/test_new_module.py"),
        )

        self.assertEqual(
            ("src/new_module.py", "tests/test_new_module.py"),
            registered.write_scope,
        )
        self.assertEqual(
            ("src/new_module.py", "tests/test_new_module.py"),
            self.index.assertions[-1][2],
        )
        self.assertTrue(self.index.assertions[-1][3])
