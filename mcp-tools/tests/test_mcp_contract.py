...
class MCPContractTests(unittest.TestCase):
    ...
    def test_tool_annotations_expose_read_and_host_confirmation_boundaries(
        ...
        self.assertIn(
            "user confirmation", tools["workflow_approval_grant"].description.lower()
        )
        self.assertTrue(
    ...
    def test_data_root_priority_and_rejections_never_use_plugin_source_or_cache(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.dict(
                ...
                self.assertEqual(
                    (root / "devkit").resolve(), server._resolve_data_root()
                )
            with patch.dict(
                os.environ, {"BUGKILLER_HOME": str(root / "legacy")}, clear=True
            ):
                self.assertEqual(
                    (root / "legacy").resolve(), server._resolve_data_root()
                )
            with patch.dict(
                os.environ, {"CODEX_HOME": str(root / "codex")}, clear=True
            ):
                self.assertEqual(
                    (root / "codex" / "2718lab-devkit").resolve(),
                    server._resolve_data_root(),
                )
            with patch.dict(
                os.environ, {"DEVKIT_HOME": str(server.PLUGIN_ROOT)}, clear=True
            ):
            ...
            with patch.dict(os.environ, {"DEVKIT_HOME": str(cache_root)}, clear=True):
    ...
    def test_shared_approval_tools_return_data_only_and_never_execute_effects(
        self,
    ) -> None:
        manifest = {
            "action": "commit",
        ...
        with (
            tempfile.TemporaryDirectory() as temporary,
            patch.dict(os.environ, {"DEVKIT_HOME": temporary}, clear=True),
        ):
            route = server.bugkiller_route(["credentials"])
            prepared = server.workflow_approval_prepare(
                manifest, "2099-01-01T00:00:00+00:00"
            )
            granted = server.workflow_approval_grant(prepared["data"]["id"])
            claimed = server.workflow_approval_claim(prepared["data"]["id"], manifest)
