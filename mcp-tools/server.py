...
from bugkiller.adapters import detect_repository
from bugkiller.approvals import ApprovalError, ApprovalManifest
from bugkiller.policy import route_case
from bugkiller.service import ApprovalEffectService
...
def _resolve_data_root() -> Path:
    ...
    if os.environ.get("BUGKILLER_HOME"):
        ...
        candidate = Path(os.environ["CODEX_HOME"]) / "bugkiller"
    else:
...
def bugkiller_detect_adapters(repository: str) -> dict[str, Any]:
    """Return structured verification command specs without executing them."""
...
def bugkiller_approval_prepare(
    ...
    """Prepare an immutable approval record; performs no external effect."""
...
def bugkiller_approval_deny(approval_id: str) -> dict[str, Any]:
    """Record denial for a prepared manifest without consuming a grant."""
...
def bugkiller_approval_claim(
    ...
    """Claim a single-use grant in the journal; never executes Git or network work."""
