...
class ContinuityCas:
    ...
    def put_verified(self, content_hash: str, byte_length: int, body: bytes) -> str:
        ...
        try:
            ...
            _safe_root(target.parent, create=False)
            ...
            try:
                ...
                os.close(descriptor)
            self._read_target(stage, content_hash, byte_length)
            try:
    ...
    @staticmethod
    def _read_target(path: Path, content_hash: str, byte_length: int) -> bytes:
        try:
            status = path.lstat()
