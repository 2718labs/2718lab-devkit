...
class AtlasStore:
    def __init__(self, database_path: str | Path, cas_root: str | Path | None = None) -> None:
        self._database_path = Path(database_path)
        self._cas_root = Path(cas_root) if cas_root is not None else self._database_path.parent / "cas"
    ...
    def _migrate(self) -> None:
        ...
        try:
            exists = self._conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='atlas_metadata'").fetchone()
            if exists is None:
                for statement in _SCHEMA.split(";"):
                    if statement.strip():
                        self._conn.execute(statement)
            current = self._conn.execute("SELECT value FROM atlas_metadata WHERE key='schema_version'").fetchone()
            if current is None:
                self._conn.execute("INSERT INTO atlas_metadata(key,value) VALUES ('schema_version','1')")
            elif current[0] != "1":
    ...
    def _node_identity(node: AtlasNode) -> str:
        return canonical_id(node.kind.value, node.payload, schema_version=node.schema_version,
                            extractor_id=node.extractor_id, extractor_version=node.extractor_version,
                            provenance=node.provenance, source_hashes=node.source_hashes)

    @staticmethod
    def _edge_identity(edge: AtlasEdge) -> str:
        return canonical_hash({"relation": edge.relation.value, "source_id": edge.source_id,
    ...
    def put_nodes(self, nodes: Iterable[AtlasNode]) -> tuple[AtlasNode, ...]:
        ...
        with self._conn:
            for node in items:
                immutable = (node.kind.value, canonical_json(node.payload), node.schema_version, node.extractor_id,
                             node.extractor_version, node.provenance, canonical_json(node.source_hashes))
                row = self._conn.execute("SELECT kind,payload_json,schema_version,extractor_id,extractor_version,provenance,source_hashes_json FROM atlas_nodes WHERE node_id=?", (node.node_id,)).fetchone()
    ...
    def put_edges(self, edges: Iterable[AtlasEdge]) -> tuple[AtlasEdge, ...]:
        ...
        for edge in items:
            ...
            endpoints = self._conn.execute("SELECT node_id,kind FROM atlas_nodes WHERE node_id IN (?,?)", (edge.source_id, edge.target_id)).fetchall()
            if len(endpoints) != 2 or dict(endpoints).get(edge.source_id) != edge.source_kind.value or dict(endpoints).get(edge.target_id) != edge.target_kind.value:
                raise StoreConflictError("edge endpoint conflict")
        with self._conn:
            for edge in items:
                immutable = (edge.source_id, edge.target_id, edge.relation.value, canonical_json(edge.payload), edge.schema_version, edge.provenance)
                row = self._conn.execute("SELECT source_id,target_id,relation,payload_json,schema_version,provenance FROM atlas_edges WHERE edge_id=?", (edge.edge_id,)).fetchone()
                if row is not None and tuple(row) != immutable:
    ...
    def put_blob(self, blob_hash: str, content: bytes, media_type: str = "application/octet-stream") -> str:
        ...
        with self._conn:
            ...
            if row is not None and tuple(row) != values:
                raise StoreConflictError("blob metadata conflict")
            if row is None:
                self._conn.execute("INSERT INTO atlas_blobs VALUES (?,?,?)", (blob_hash, *values))
        return blob_hash
    ...
    def put_packet(self, packet: ImplementationPacket, *, created_at: str = "") -> str:
        payload = canonical_json(packet.to_dict()); digest = canonical_hash(packet.to_dict())
        with self._conn:
            row = self._conn.execute("SELECT snapshot_id,packet_json,packet_hash FROM atlas_packet_receipts WHERE packet_id=?", (packet.packet_id,)).fetchone()
            values = (packet.snapshot_id, payload, digest)
            if row is not None and tuple(row) != values: raise StoreConflictError("packet receipt conflict")
            if row is None: self._conn.execute("INSERT INTO atlas_packet_receipts VALUES (?,?,?,?,?)", (packet.packet_id, *values, created_at))
    ...
    def put_ingestion_receipt(self, receipt: IngestionReceipt) -> str:
        ...
        with self._conn:
            row = self._conn.execute("SELECT payload_hash,status,episode_id,recipe_id,reasons_json,created_at FROM atlas_ingestion_receipts WHERE ingestion_key=?", (receipt.ingestion_key,)).fetchone()
            if row is not None and tuple(row) != values: raise StoreConflictError("ingestion receipt conflict")
            if row is None: self._conn.execute("INSERT INTO atlas_ingestion_receipts VALUES (?,?,?,?,?,?,?)", (receipt.ingestion_key, *values))
    ...
    def recipes_for_intent(self, intent_id: str, *, language: str | None = None, framework: str | None = None, state: str | None = None) -> tuple[str, ...]:
        ...
        for column, value in (("language", language), ("framework", framework), ("state", state)):
            if value is not None: clauses.append(column + "=?"); args.append(value)
        return tuple(row[0] for row in self._conn.execute("SELECT recipe_id FROM atlas_recipes WHERE " + " AND ".join(clauses) + " ORDER BY recipe_id", args))
    ...
    def graph_query(self, root_node_ids: Iterable[str], *, max_nodes: int = MAX_GRAPH_NODES, max_edges: int = MAX_GRAPH_EDGES, max_depth: int = MAX_GRAPH_DEPTH, byte_budget: int = MAX_PACKET_BYTES, node_kinds: Iterable[NodeKind] | None = None, relations: Iterable[EdgeRelation] | None = None) -> GraphQueryResult:
        ...
        for root in frontier:
            if self._conn.execute("SELECT 1 FROM atlas_nodes WHERE node_id=?", (root,)).fetchone() is not None:
                if len(chosen) >= max_nodes: truncated = True; break
                chosen.add(root)
        selected_edges: set[str] = set()
        for _depth in range(max_depth):
            next_frontier: set[str] = set()
            for node_id in sorted(frontier):
                rows = self._conn.execute("SELECT edge_id,source_id,target_id,relation FROM atlas_edges WHERE source_id=? OR target_id=? ORDER BY edge_id", (node_id, node_id)).fetchall()
                for edge_id, source, target, relation in rows:
                    other = target if source == node_id else source
                    if allowed_relations is not None and relation not in allowed_relations: continue
                    ...
                    if other not in chosen and len(chosen) >= max_nodes: truncated = True; continue
