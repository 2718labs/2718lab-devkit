...
2. 校验器会自动识别 AstrBot `metadata.yaml` 或 Codex `.codex-plugin/plugin.json`。逐条解读输出,重点盯这些坑:`version` 非三段号、`repo` 带 `.git`、`astrbot_version` 缺引号或带 `v`、剔除 `.git`/`__pycache__` 后打包体积 > 16MB、缺 LICENSE/README/CHANGELOG/CI。
