...
2. 复制模板:库(lib)用 `${CLAUDE_PLUGIN_ROOT}/skills/python-engineering/assets/templates/pyproject.toml`,应用(app)用 `pyproject-app.toml`;再附 `.pre-commit-config.yaml`、`.python-version`。
...
5. 交付前运行 `python "${CLAUDE_PLUGIN_ROOT}/skills/python-engineering/scripts/validate_project.py" <项目目录>`。
