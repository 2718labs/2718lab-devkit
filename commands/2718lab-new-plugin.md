...
2. 从 `skills/astrbot-plugin-dev/assets/templates/` 复制整套起步文件到新插件目录。目录名 = 插件名,须 `astrbot_plugin_` 前缀、全小写、合法模块名(无连字符/大写/空格)。
...
5. 交付前运行 `python skills/astrbot-plugin-dev/scripts/validate_plugin.py <插件目录>`,**0 错误**才算完。
