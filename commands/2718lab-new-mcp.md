...
3. 复制模板:(A) `${CLAUDE_PLUGIN_ROOT}/skills/mcp-server-dev/assets/templates/server_sdk.py`,(B) `server_standalone.py`,都配上同目录的 `pyproject.toml`。
...
6. 交付前运行 `python "${CLAUDE_PLUGIN_ROOT}/skills/mcp-server-dev/scripts/validate_mcp_server.py" <server目录或文件>`。
